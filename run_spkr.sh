...
do
	...
	do
		for surface in norm
		do
			model_name=spkr_gaussian_tsne_layer-${layer}_coeff-${ccoeff}-init_${surface}
			echo $model_name
			mkdir -p $dir/$model_name/
			mkdir -p $dir/$model_name/pretrain_params
			mkdir -p $dir/$model_name/log
			ls $dir/$model_name/tmp.dnn.pkl
			#[ -f $dir/$model_name/tmp.dnn.pkl ] || \
				python -u $TK_DIR/train_spkr.py \
				--frames-files			 ${frame_files[@]:1}   \
				--labels-files			 ${label_files[@]:1}   \
				--phoneme-files			 ${phoneme_files[@]:1}   \
			...
			python -u $TK_DIR/run_stats.py \
				...
				--model-location $dir/$model_name/tmp.dnn.pkl \
				--constraint-surface $surface \
				--structure $structure \
