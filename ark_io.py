...
def read_kaldi_matrix(buffer, skip_binary_preamble=False):
    ...
    if skip_binary_preamble:
        # read 0B{F,D}{V,M,C}[space], function tested for 0BFM types only
        descr = struct.unpack('<ccc', buffer.read(5))
        repr_type = descr[0]
        cont_type = descr[1]
    else:
        # read 0B{F,D}{V,M,C}[space], function tested for 0BFM types only
        descr = struct.unpack('<xcccc', buffer.read(5))
        binary_mode = descr[0]
        repr_type = descr[1]
        cont_type = descr[2]
