...
shared_structure = config.option("shared_structure","Structure for shared portion.", type=config.structure)
...
@pooling_method
def build_speaker_inferer(P,input_dimension,layer_size,speaker_latent_size,pooling_method):
...
@acoustic_structure
@speaker_structure
def build_encoder(P,input_dimension,shared_structure,acoustic_structure,speaker_structure):
...
#        shared_rep = shared_transform(X)[-1]
        return shared2speaker(X,mask)
    
    def acoustic_encode(X,utterance_speaker):
#        shared_rep = shared_transform(X)[-1]
        return shared2acoustic([X,utterance_speaker])
