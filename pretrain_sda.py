...
def corrupt(x, corr=0.2):
    corr_x = theano_rng.binomial(
        size=x.shape, n=1, p=1 - corr, dtype=theano.config.floatX) * x
...
if __name__ == "__main__":
    ...
    Ws = [P["W_classifier_input_0"]] + [P["W_classifier_%d" % i]
                                        for i in xrange(1, len(layers))]
    bs = [P["b_classifier_input"]] + [P["b_classifier_%d" % i]
                                      for i in xrange(1, len(layers))]
        ...
        train_fns.append(
            chunk.build_trainer(
                ...
                updates=updates.momentum(
                    parameters, gradients, learning_rate=lr),
