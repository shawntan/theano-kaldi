...
if __name__ == "__main__":
    ...
    P = Parameters()
    training_costs = utterance_vae.build(P)
    ...
    loss = cost +\
            (0.5 / float(frame_count)) * sum(
                    T.sum(T.sqr(w)) for w in parameters \
                                if w.name not in (
                                    'b_speaker_encoder_pooled_mean',
                                    'b_speaker_encoder_pooled_std',
                                    'b_acoustic_encoder_mean',
                                    'b_acoustic_encoder_std',
                                    'b_decode_mean',
                                    'b_decode_std'))
    from pprint import pprint
    pprint(sorted(w.name for w in parameters if w.name not in (
                                    'b_speaker_encoder_pooled_mean',
                                    'b_speaker_encoder_pooled_std',
                                    'b_acoustic_encoder_mean',
                                    'b_acoustic_encoder_std',
