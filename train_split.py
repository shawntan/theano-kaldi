...
config.parser.add_argument(
		...
		required = True,
		type = str,
		help = "Filter utterance ID prefix"
	)
...
if __name__ == "__main__":
	...
	for epoch in xrange(config.max_epochs):
		total_errors = 0
		total_frames = 0
		val_split_streams = [ data_io.stream(f,l,p,with_name=True) for f,l,p in izip(val_frames_files,val_labels_files,val_phonemes_files) ]
		val_stream = chain(*val_split_streams)
		val_stream = ( (f,l,p) for n,f,l,p in val_stream if n.startswith(config.args.utt_id_filter) )
		count = 0
		for f,l,p in val_stream:
		...
		split_streams = [ data_io.stream(f,l,p,with_name=True) for f,l,p in izip(frames_files,labels_files,phoneme_files) ]
		stream = chain(*split_streams)
		stream = ( (f,l,p) for n,f,l,p in stream if n.startswith(config.args.utt_id_filter) )
		total_frames = 0
