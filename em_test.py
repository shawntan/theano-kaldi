...
def build(params,name,phonemes,components,rows,cols,spread=1):
	n_hidden = rows * cols
	"""
	...
	"""
	mean_arr = (np.random.rand(phonemes,components,2) * np.array([[[rows,cols]]])).astype(np.float32)

	phoneme_mixtures = theano.shared(
				(np.ones((phonemes,components))/components).astype(np.float32)
			)                       # phonemes x components
	phoneme_means    = theano.shared(mean_arr)  # phonemes x components x 2
	phoneme_var      = theano.shared(
			np.array(phonemes * [
				components * [ spread * np.eye(2) ]
				]).astype(np.float32)
	...
	points = theano.shared(np.dstack(np.meshgrid(
		...
		)).reshape(n_hidden,2).astype(np.float32))                                            # n_hidden x 2
	...
	gaussians = T.exp(- np.float32(0.5) * score)\
			/ (T.sqrt(np.float32(2 * np.pi**2) * phoneme_determinants)).dimshuffle(0,1,'x')
	...
	def constraint(hidden,phonemes):
		...
		output = -T.mean(T.sum(surface * T.log(norm_hidden),axis=1))
		return output
...
def mask(params,name,phonemes,components,rows,cols):
	n_hidden = rows * cols
	...
	def mask(phoneme):
		mog  = T.sum(weighted_gaussians,axis=1)
		mask = 1 - mog[phoneme]
		return mask
