...
config.parser.add_argument(
		...
		required = True,
		type = str,
		help = ".pklgz files containing pickled (name,phonemes) pairs for training"
	)
...
def speaker_indexify(spkrs,stream):
	spkr2idx = { spkr:i for i,spkr in enumerate(spkrs) }
	for x in stream:
		spkr = x[0][:5]
		phn = x[-1]
		idxs = np.empty(x[1].shape[0],dtype=np.int32)
		idxs.fill(spkr2idx[spkr])
		idxs[phn == 1] = -1
		yield x[1:-1] + (idxs,)
...
if __name__ == "__main__":
	...
	phonemes_files = config.args.phoneme_files
	val_phonemes_file = config.args.val_phonemes_file

	minibatch_size = config.minibatch
	
	gaussian_ctr = pickle.load(open('spkr_gaussian_ctr.pkl'))
	print gaussian_ctr.shape
	spkrs = pickle.load(open('spkr_order.pkl'))
	print spkrs[:10]
	...
	if config.args.constraint_coeff > 0:
		...
		gmm_constraint,em_updates = em.build(
			...
			phonemes = gaussian_ctr.shape[0],
			components = 1,
			rows = 32,cols = 32
		)

		gmm_params["phoneme_means_%d"%config.args.constraint_layer].set_value(
				gaussian_ctr.reshape(gaussian_ctr.shape[0],1,2).astype(np.float32))
...
#		update_params = theano.function(inputs=[],    updates = param_updates) 
	...
	for epoch in xrange(config.max_epochs):
		total_errors = 0
		total_frames = 0
		test_stream = data_io.stream(val_frames_file,val_labels_file,val_phonemes_file,with_name=True)
		test_stream = speaker_indexify(spkrs,test_stream)
		for f,l,p in test_stream:
		...
		stream = speaker_indexify(spkrs,stream)
