    ...
    for frames, speaker_id in stream:
        ...
        if batch_ptr == batch_size:
            ...
            batch_frames = np.empty(
                    (batch_size,min_frames,frames.shape[1]),dtype=np.float32)
            batch_speaker_id = np.array([s for _,s in batch_buffer],dtype=np.int32)

            idx = np.arange(min_frames)
            for i,(f,s) in enumerate(batch_buffer):
                if f.shape[0] > min_frames:
                    interval  = f.shape[0]/float(min_frames)
                    selection = np.floor(
                            idx * interval + \
                                np.random.uniform(interval,size=min_frames)).astype(np.int32)
                    batch_frames[i,:,:] = f[selection]
                else:
                    batch_frames[i,:,:] = f

            yield batch_frames,batch_speaker_id
            batch_ptr = 0
    batch_buffer = batch_buffer[:batch_ptr]
    for i,(f,s) in enumerate(batch_buffer):
        selected_frames = np.random.choice(f.shape[0],min_frames,replace=False)
        batch_frames[i,:,:] = f[selected_frames]
    yield batch_frames,batch_speaker_id
...
def build_data_stream(frames_files,utt2spk_file,context=5):
    streams = [ data_io.buffered_random(s)
...
if __name__ == "__main__":
    utterance_count = sum(
            1 for _ in chain(*build_data_stream(
    ...
    loss = cost +\
            ...
            (0.5/utterance_count) * sum(
                    ...
                    if not w.name.startswith('b') 
                    or not w.name == 'speaker_vector'
                )

    logging.debug('Calculating gradients...')
    gradients = T.grad(loss,wrt=parameters)
    gradients = updates.clip(20)(gradients)
    ...
    for _ in xrange(200):
        ...
        for batch_frames,batch_speaker_id in build_data_stream(config.args.frames_files,config.args.utt2spk_file):
            print batch_speaker_id, batch_frames.shape[1]
            print train(batch_frames,batch_speaker_id)
